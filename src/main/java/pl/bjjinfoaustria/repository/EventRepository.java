...
public interface EventRepository extends JpaRepository<Event, Long>{
	...
	@Query(value="select * from Event e where (e.typeOfEvent = :camp or e.typeOfEvent = :seminar or e.typeOfEvent = :competition) "
			+ "and ((e.status = 'ACTIVE' or e.status='RELEASED' ) or e.organizer = :organizer)",nativeQuery = true)
	...
	@Query(value="select * from Event e where (e.typeOfEvent = :camp or e.typeOfEvent = :seminar or e.typeOfEvent = :competition) "
			+ "and (e.status = 'ACTIVE' or e.status='RELEASED' )",nativeQuery = true)
	...
	@Query(value="select * from Event e where (e.status = 'ACTIVE' or e.status='RELEASED' ) or e.organizer = :organizer",nativeQuery = true)
	public List<Event> findAllActiveOrByOrganizer(@Param ("organizer") String organizer);
	
	@Query(value="select * from Event e where (e.status = 'ACTIVE' or e.status='RELEASED' )",nativeQuery = true)
